...
const SU = JSON.parse(
  fs.readFileSync("../output/consolidated-sampling-units.geojson")
);
...
converter.json2csv(
  ...
  function (err, csv) {
    ...
    fs.writeFile("../output/consolidated-names-list.csv", csv, function (err) {
      if (err) {
...
fs.writeFileSync("../output/consolidated-names.json", JSON.stringify(names));
fs.writeFileSync("../output/p-consolidated-names.json", JSON.stringify(p_names));
